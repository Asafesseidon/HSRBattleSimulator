...
class Ally extends Personagem{
    ...
    constructor(life:number,weapon: LightCone, basicAttack:BasicAttack, skill:Skill, attack:number, defense: number, speed:number,id:string, ultimate:Ultimate, talent:Talent, fuA:FuA){
        super(life,basicAttack, skill, attack, defense, speed, id, ultimate, talent);

        this.lightcone=weapon;
        this.fuA=fuA;
    }
    public getAllyLC(){
...
class Enemy extends Personagem{
    ...
    constructor(toughness:number, life:number, basicAttack:BasicAttack, skill:Skill, attack:number, defense: number, speed:number,id:string, ultimate:Ultimate, talent:Talent,currentToughness:number, isBoss:boolean, isBroken:boolean){
        super(life,basicAttack, skill, attack, defense, speed, id, ultimate, talent);
...
function chars(id:number, iteration:number):Personagem[] | number{
    ...
    if (id == 0) {
        ...
        new Ultimate('e',110, 'buff', 500, 0, 'AoE', 'Increases energy and ATK', 10), new Talent('a', 'buff', 'Heal','Single Target', 10),
        new FuA('', '','','',0))]; //Huohuo
        iteration++;
        ...
        new Ultimate('q', 120, 'attack', 470, 0, 'AoE', "Apply Shock to all targets and Trigger all DoT's currently on them", 75), new Talent('a', 'attack', 'FuA+Debuff','Single Target', 10), 
        new FuA('a', '','','',100))]; //Kafka
        iteration++;
        ...
        new Ultimate('q', 120, 'attack', 470, 0, 'AoE', 'Applies Epiphany and Treats Arcana as Bleed, Burn, Wind Shear and Shock', 10), new Talent('a', 'buff', 'a','Single Target', 10),
        new FuA('', '','','',0))]; //Black Swan
        iteration++;
        ...
        new Ultimate('q', 120, 'buff', 470, 0, 'AoE', 'Applies RESPEN', 25), new Talent('a', 'buff', 'Speed Buff+ Extra Break Dmg','AoE', 10),
        new FuA('', '','','',0))]; //Ruan Mei
        iteration++;
        ...
        new Ultimate('q', 120, 'attack', 470, 0, 'Single Target', 'Applies Crit DMG Vulnerabity to Target and gain stacks for FuA', 10), new Talent('a', 'buff', 'a','Single Target', 10),
        new FuA('a', 'attack','Stacks reach 7','AoE',100))]; //Aventurine
        iteration++;
        ...
        new Ultimate('q', 120, 'attack', 470, 0, 'Single Target', 'Buffs Crit Rate and enters Soulsteel Sync state', 50), new Talent('a', 'buff', 'a','Single Target', 10),
        new FuA('a', 'attack','60% chance to trigger everytime skill is used in Soulsteel Sync state','Single Target',100))]; //Yanqing
        iteration++;
